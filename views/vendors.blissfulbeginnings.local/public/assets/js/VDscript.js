...
function render() {
  ...
  const navigateEditProfileButton = document.querySelector(
    ".view-packages-button"
  );
  const navigatePackages = document.querySelector(".navigate-to-packages");
 ...
 .unavailable-day::after {
  ...
  navigatePackages.addEventListener("click", function (e) {
    e.preventDefault();

    const urlpack =
      "http://vendors.blissfulbeginnings.com/packages/" + vendorID;
    console.log(urlpack);
    window.location.href = urlpack;
  });
  ...
  function loadCards(cards) {
    ...
    const cardsInGroup = cards.map((card) => createCard(card)).join("");
  ...
  if (calendarModalContainer2 && confirmBtn2) {
    ...
    if (editProfile && editModalContainer) {
      ...
      document.addEventListener("keydown", (event) => {
        if (
          event.key === "Escape" &&
          editModalContainer.classList.contains("show")
        ) {
          closeEditModal();
        }
      });
