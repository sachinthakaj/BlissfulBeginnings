    ...
    deleteButton.addEventListener('click', () => {
        ...
        .then(response => {
            if (response.status === 200) {
                ...
                closeModal();
                setTimeout(() => window.location.href = '/florists', 1000);
            } 
...
async function notFund() {
  ...
  scrollContainer.appendChild(notFund);
    ...
    async function fetchFlorists() {
      try {
        const response = await fetch("/get-floristlist/", {
          method: "GET",
          headers: {
            'Authorization': `Bearer ${localStorage.getItem('authToken')}`,
                'Content-Type': 'application/json'
    ...
    async function render() {
      const data = await fetchFlorists();
      if (data.length === 0) {
        notFund();
        return;
      } else {
    ...
    document.addEventListener("DOMContentLoaded", () => {
      ...
      if (searchButton) {
        searchButton.addEventListener("click", searchFlorists);
      } else {
